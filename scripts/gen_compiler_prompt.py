...
FOOTER = """Output ONLY the compiled Markdown containing the valid `# [State]` and `## [Transitions]` structures. Do not wrap the output in markdown code blocks unless the input draft itself was wrapped.
...
# spec 驱动的规则生成
# ---------------------------------------------------------------------------
def render_state_rules(spec: Dict[str, Any]) -> str:
    st = spec["sections"]["state"]
    meta = st["metadata"]
    return f"""1. Every State must be declared as a top-level heading: `{st['heading']}`.
...
def render_section_rules(spec: Dict[str, Any]) -> str:
    s = spec["sections"]
    return f"""3. Preserve the global instructions (text before the first top-level section) and each state's task instructions unchanged, except for formatting them into standard markdown blocks.
4. Recognized top-level sections are: `{s['config']['heading']}` (engine params like `max_loops`), `{s['io']['heading']}` (reader/writer), `{s['tools']['heading']}` (tool declarations), and `{s['state']['heading']}` (state nodes). Preserve them exactly as-is.
...
def build_prompt(spec: Dict[str, Any]) -> str:
    rules = "\n".join(
        [RULES_INTRO, render_state_rules(spec), render_transition_rules(spec), render_section_rules(spec)]
    )
    return f"{HEADER}\n\n{rules}\n\n{FOOTER}"
